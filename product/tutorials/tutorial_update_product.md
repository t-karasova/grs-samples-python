...
The product object with updated fields is already preset in <walkthrough-editor-select-regex filePath="cloudshell_open/grs-samples-python/product/update_product.py" regex="# generate product for update">update_product.py</walkthrough-editor-select-regex>:
```
{
...
1. Open <walkthrough-editor-select-regex filePath="cloudshell_open/grs-samples-python/product/update_product.py" regex="# get update product request">update_product.py</walkthrough-editor-select-regex>.
...
The `update_mask` in the `UpdateProductRequest` method indicates which fields in the provided product should be updated. If it isn't set, all supported fields are updated.
...
1. To update some fields in the original product, add the `update_mask` field in the `get_update_product_request()` method:
    ```
...
As you can see, only the product fields listed in the  `update_mask` were updated. Other fields weren't changed even if they were provided in the product object in the `UpdateProductRequest` method.
